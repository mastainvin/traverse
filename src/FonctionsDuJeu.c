...
bool autorisation_bord(saves_value *partie, coordInt depart, coordInt arrive, bool first_recusive)
{
  ...
  if (partie->move.player == 1 || partie->move.player == 2)
  {
    if (arrive.y == 0)
    {
      ensemblePossible[0] = 1;
      ensemblePossible[1] = 2;
      ensemblePossible[2] = 4;
      ensemblePossible[3] = 6;
      ensemblePossible[4] = 7;
    }
    else if (arrive.y == 9)
    {
      ensemblePossible[0] = 0;
      ensemblePossible[1] = 1;
      ensemblePossible[2] = 3;
      ensemblePossible[3] = 5;
      ensemblePossible[4] = 6;
    }
  }
  else if (partie->move.player == 3 || partie->move.player == 4)
  {
    if (arrive.x == 0)
    {
      ensemblePossible[0] = 3;
      ensemblePossible[1] = 4;
      ensemblePossible[2] = 5;
      ensemblePossible[3] = 6;
      ensemblePossible[4] = 7;
    }
    else if (arrive.x == 9)
    {
      ensemblePossible[0] = 0;
      ensemblePossible[1] = 1;
      ensemblePossible[2] = 2;
      ensemblePossible[3] = 3;
      ensemblePossible[4] = 4;
    }
  }
  ...
  if (ensemblePossible[0] != -1)
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      ...
      remplirTabRest(tab_temp, restriction_temp, &partie->move, arrive.x, arrive.y);

      // si toutes les conditions de règles sont vérifiés, on peut alors accomplir récursive la condition
      if (autorisation(arrive.x, arrive.y, arrive_temp.x, arrive_temp.y, partie->tab) && restriction_temp[ensemblePossible[i]])
        return autorisation_bord(partie, arrive, arrive_temp, false);
    }
  }
  else
  {
    ...
    *coord_temp = selectionMove(partie->tab, x, y, plateau);
    ...
    if (coord_temp->x >= 0 && coord_temp->y >= 0)
    {
      ...
      */
      ...
      int signe_i_temp = (coord_temp->x - partie->selectedBox.x) / (2.0 * distance_i_temp);
      int signe_j_temp = (coord_temp->y - partie->selectedBox.y) / (2.0 * distance_j_temp);
      ...
      if (autorisation(partie->selectedBox.x, partie->selectedBox.y, coord_temp->x, coord_temp->y, partie->tab) && partie->restriction[elementCorrespondant(i_temp, j_temp)] && autorisation_bord(partie, partie->selectedBox, *coord_temp, true))
      {
        ...
        for (int i = 0; i < 8; i++)
        {
          ...
          if (partie->restriction[i] && autorisation_bord(partie, partie->selectedBox, coord_temp_nextTurn, true))
...
fin_partie conditionDeFin(saves_value *partie)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    {
      // Joueur 1
      if (partie->tab[1 + i][0].joueur == 1)
        accJoueur1++;
      // Joueur 2
      if (partie->tab[1 + i][9].joueur == 2)
        accJoueur2++;
      // Joueur 3
      if (partie->tab[9][1 + i].joueur == 3)
        accJoueur3++;
      // Joueur 4
      if (partie->tab[0][1 + i].joueur == 4)
        accJoueur4++;
    }
  }
  ...
  if (testFin.joueur == 1 || testFin.joueur == 2 || testFin.joueur == 3 || testFin.joueur == 4)
    testFin.fin = 2;
