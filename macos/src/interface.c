...
void menu_secondaire(SDL_Renderer *renderer, SDL_Window *window, SDL_Rect *fenetre, menu_bouton *selection, coord coordCurseur, coord coordClic, saves_value *partie, fleche *bouton_fleche, config_type *config, saves *li)
{
  ...
  case jouer:
  {
    ...
    if (coordClic.y >= 300 && coordClic.y <= 400)
    {
      ...
      if (coordClic.x >= (fenetre->w + 500) / 2 - 100 && coordClic.x <= (fenetre->w + 500) / 2 && (partie->parametre.ordis == 0))
        partie->parametre.ordis++;
    }
    ...
    if (partie->parametre.joueurs + partie->parametre.ordis > 2)
      partie->parametre.ordis = 0;
    ...
    if (coordClic.y >= 450 && coordClic.y <= 500 && partie->parametre.ordis == 1)
    {
      if (coordClic.x >= (fenetre->w + 250) / 2 - 375 && coordClic.x <= (fenetre->w + 250) / 2 - 175 && (partie->parametre.niveau == 2))
        partie->parametre.niveau = 1;

      if (coordClic.x >= (fenetre->w + 250) / 2 - 125 && coordClic.x <= (fenetre->w + 250) / 2 + 125 && (partie->parametre.niveau == 1))
        partie->parametre.niveau = 2;
    }

    if (partie->parametre.niveau == 1)
    {
      afficherImage("menu_secondaire/facile_1.bmp", (fenetre->w + 250) / 2 - 375, 400, renderer, window, fenetre, *config);
      afficherImage("menu_secondaire/difficile_0.bmp", (fenetre->w + 250) / 2 - 125, 400, renderer, window, fenetre, *config);
    }
    else
    {
      afficherImage("menu_secondaire/difficile_1.bmp", (fenetre->w + 250) / 2 - 125, 400, renderer, window, fenetre, *config);
      afficherImage("menu_secondaire/facile_0.bmp", (fenetre->w + 250) / 2 - 375, 400, renderer, window, fenetre, *config);
    }

    if (partie->parametre.ordis != 1)
    {
      afficherImage("menu_secondaire/bouton_indisponible.bmp", (fenetre->w + 250) / 2 - 375, 400, renderer, window, fenetre, *config);
      afficherImage("menu_secondaire/bouton_indisponible.bmp", (fenetre->w + 250) / 2 - 125, 400, renderer, window, fenetre, *config);
    }
  ...
  case rejouer: // Si l'utilisateur à cliqué sur charger une partie
  {
    ...
    // création de fond dynamoques en fonction du nombre de sauvegardes
    ...
    saves_value *ancienne_partie = NULL;
    ...
    if (*selection == commencer)
    {
      ...
      copierTab(ancienne_partie->tab, partie->tab);
    ...
    if (bouton_fleche->element > 1)
    {
      ...
      if (bouton_fleche->haut || (coordClic.x >= 150 && coordClic.x <= 200 && coordClic.y >= 115 && coordClic.y <= 165))
    ...
    if (bouton_fleche->element < nombre_element - 2)
    {
      ...
      if (bouton_fleche->bas || (coordClic.x >= 150 && coordClic.x <= 200 && coordClic.y >= 620 && coordClic.y <= 670))
        bouton_fleche->element++;
    }

  }
  // Le programme lancera alors la partie au même endroit que l'ancienne
  break;
  ...
  case options:
  {
    ...
    if (config->theme == 1)
    {
      ...
      afficherImage("/menu_secondaire/starcraft_0.bmp", CENTER, 200, renderer, window, fenetre, *config);                       // à changer pour starcraft
      afficherImage("/menu_secondaire/futuriste_0.bmp", (fenetre->w - 150) / 2 + 125, 200, renderer, window, fenetre, *config); // à changer pour troisème thème
    }
    else if (config->theme == 2)
    {
      afficherImage("/menu_secondaire/medieval_0.bmp", (fenetre->w - 150) / 2 - 225, 200, renderer, window, fenetre, *config);
      afficherImage("/menu_secondaire/starcraft_1.bmp", CENTER, 200, renderer, window, fenetre, *config);                       // à changer pour starcraft
      afficherImage("/menu_secondaire/futuriste_0.bmp", (fenetre->w - 150) / 2 + 125, 200, renderer, window, fenetre, *config); // à changer pour troisème thème
    }
    else
    {
      afficherImage("/menu_secondaire/medieval_0.bmp", (fenetre->w - 150) / 2 - 225, 200, renderer, window, fenetre, *config);
      afficherImage("/menu_secondaire/starcraft_0.bmp", CENTER, 200, renderer, window, fenetre, *config);                       // à changer pour starcraft
      afficherImage("/menu_secondaire/futuriste_1.bmp", (fenetre->w - 150) / 2 + 125, 200, renderer, window, fenetre, *config); // à changer pour troisème thème
    }
  ...
  case regles:
  {
    ...
    afficherImage("menu_secondaire/regles.bmp", CENTER, CENTER, renderer, window, fenetre, *config);
  }
  break;
  default:
    break;
...
location menu_save(SDL_Renderer *renderer, SDL_Window *window, SDL_Rect *fenetre, cell tab[10][10], coord coordCurseur, coord coordClic, bool *inPause, nom_sauvegarde *save_name, bool *saving, config_type config)
{
  ...
  SDL_Color white = {255, 255, 255};
  afficherTexte("ATTENTION, si le nom de votre sauvegarde est vide,", CENTER, 600, white, 30, renderer, window, fenetre);
  afficherTexte("la partie ne sera pas sauvegarder !", CENTER, 650, white, 30, renderer, window, fenetre);
...
void afficherImage(char *lienImage, float x, float y, SDL_Renderer *renderer, SDL_Window *window, SDL_Rect *fenetre, config_type config)
{
  ...
  if (config.theme == 3)
    sprintf(lien_Theme, "./../img/futuriste/%s", lienImage);
